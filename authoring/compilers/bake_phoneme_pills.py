"""Bake phoneme pills from the unified token inventory → cartridges/factory/.

Reads `cartridges/factory/_source/token_inventory_unified_v2.json` and
the shape files under `cartridges/factory/_source/shapes/<cat>/<key>.json`,
and rebuilds the shipping pill directory `cartridges/factory/` alongside
a single manifest at `cartridges/factory/manifest.json` that maps short
labels to cartridge paths.

No re-authoring, no schema transformation. The shape files under
`cartridges/factory/_source/shapes/` are already `compiled-v1` cartridges
...
the top level of `cartridges/factory/` so the runtime has a stable flat
...
rebuild of the flat pill layout in `cartridges/factory/` from
`cartridges/factory/_source/`.
"""
