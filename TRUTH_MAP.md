...
| Tool | Writes what |
|---|---|
| `authoring/compilers/bake_hedz_const.py` | `runtime/trench-core/src/hedz_rom.rs`, `runtime/trench-core/src/hedz_golden.rs` — hardcoded Talking Hedz cartridge + golden impulse vector for the cross-language parity test |
| `authoring/compilers/bake_phoneme_pills.py` | `cartridges/factory/<category>/<key>.json` + `cartridges/factory/manifest.json` — shipping pill layout baked from `cartridges/factory/_source/` |
| `authoring/compilers/compile_raw.py` | `compiled-v1` cartridges from the internal `raw-stage-v1` authoring surface. Canonical contract: `docs/superpowers/specs/compiler_raw_to_cartridge.md`. Mechanical format bridge only — shipping acceptance comes from `BODIES.md`, not EX3 null parity. |
| `authoring/compilers/parity_null.py` | P2K parity gate consumed by `./check` |
| `authoring/compilers/trench_eos_pipeline_compiler.py` | Internal EOS encoded-surface harness. Default out: `cartridges/factory/generated/live_eos_surface.json`. Reachable via root launcher `trench_eos_pipeline_compiler.py`. Not a shipping cartridge writer; see `authoring/FILTER_WORK.md` "Current Active Filter Path". |
